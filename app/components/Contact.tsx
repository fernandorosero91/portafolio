        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-14">
          ...
          <div ref={iRef} className={`space-y-4 scroll-reveal ${iv ? 'visible' : ''}`}>
            ...
            <div className="flex gap-3 pt-4">
              {socialLinks.filter(s => s.name !== 'YouTube').map(s => (
                <a key={s.name} href={s.href} target="_blank" rel="noopener noreferrer" aria-label={s.name} className="btn-social w-10 h-10 rounded-full flex items-center justify-center text-white" style={{ backgroundColor: s.bg }}>
                  <svg viewBox="0 0 24 24" fill="currentColor" className="w-5 h-5"><path d={s.icon} /></svg>
                </a>
              ))}
            </div>
          </div>

...
function ProjectCard({ project, index, onDemo }: { project: typeof projects[0]; index: number; onDemo: (p: typeof projects[0]) => void }) {
  const { t } = useLanguage();
    ...
    <section id="projects" className="scroll-mt-20 py-20 px-6 lg:px-8" style={{ backgroundColor: 'var(--bg-card)' }}>
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-7">
          {projects.map((p, i) => <ProjectCard key={i} project={p} index={i} onDemo={setSelected} />)}
        </div>
      </div>

...
export default function WhatsAppButton() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (contact) {
        setIsVisible(scrollPos > 300 && scrollPos < contact.offsetTop - window.innerHeight);
      } else {
  ...
  return (
    <button
      onClick={() => window.open(personalInfo.social.whatsapp, '_blank', 'noopener,noreferrer')}
      className={`fixed bottom-5 right-[4.5rem] z-[999] w-11 h-11 bg-[#25D366] text-white rounded-full flex items-center justify-center shadow-lg transition-all duration-300 group ${
        isVisible
